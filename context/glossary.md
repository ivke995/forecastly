...
- **CurrentWeatherCard**: The reusable display-only Client Component in `components/weather/CurrentWeatherCard.tsx`; it renders a shadcn Card with current weather conditions (city, temperature, feels like, humidity, wind, weather icon/description) and optional current-risk badges received via typed props — no API calls or side effects.
...
- **ForecastTrendChart component**: The reusable display-only Client Component in `components/weather/ForecastTrendChart.tsx`; it renders dependency-free SVG temperature and precipitation trend visualizations from typed series props, using the selected `TemperatureUnit` for temperature display.
